 ...
 * 2026-05-11：新增 interactiveOnTurnStart，玩家可手动选相邻友军
 */
...
export const skill_guhe_juyuan: SkillRegistration = {
  ...
  interactiveOnTurnStart: {
    promptTitle: '炼药·聚元炉',
    promptBody: '行动开始前可指定 1 名相邻友军，本行动轮进攻后可重投 1 次骰子取较高。是否发动？',
    collectChoices: (self, engine) => {
      ...
      // 不需要选属性
      return adj.map((u) => ({ targetId: u.id }));
    },
    apply: (self, target, _stat, engine) => {
      ...
      engine.emit(
        ...
        `炼药·聚元炉：${target.name} 本轮进攻后可重投 1 次（玩家选择）`,
        {
          actorId: self.id,
          targetIds: [target.id],
          skillId: 'sr_guhe.battle',

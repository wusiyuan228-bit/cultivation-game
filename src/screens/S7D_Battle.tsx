...
小轮次 2 结束 → 大回合末水晶结算 → 进入下一大回合。`}</p>
        ...
        <div className={styles.ruleSec}>
          <h4>🚶 移动规则</h4>
          <p>{`每行动轮的移动步数上限 = 心境值（mnd）。
只能向上 / 下 / 左 / 右四方向移动，斜向不计入路径；每移动 1 格消耗 1 点心境。
我方与敌方所有存活棋子均视为障碍物，不能跨越也不能停留同格。
障碍地形（山石、河流非桥段）一律阻挡。
本回合内可在范围内多次试走，最终落脚后才结算消耗。`}</p>
        </div>

        <div className={styles.ruleSec}>
          <h4>🎲 攻击判定 & 克制</h4>
          <p>{`双方同时投掷二面骰（每颗点数 0/1/2），骰子数量 = 各自修为值。
伤害 = 攻方点数和 − 守方点数和 + 技能修正 + 克制加成（最少 1 点保底）。
